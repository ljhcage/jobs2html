...
def getUlist(url, output, handle, inforL, urllist):
    reg = urllib.request.Request(url, None, header)
    for i in range(4):
        try:
            urlcontent = urllib.request.urlopen(reg).read()  # .decode("gbk").encode('utf-8')
        except urllib.error.HTTPError as e:
            if (e.args[0].errno == 11004 or e.args[0].errno == 10060) and i <3:
                time.sleep(1)
                continue
            else:
                print("times reach")
    ...
    for i in range(3, target.__len__(), 2):  # 循环每一个宣讲会
        ...
        strtime = time.strftime('%m{0}%d{1} %H:%M',ttime).format(*'月日')#pytho3中的strftim是一个C语言库，无法处理Unicode字符，只能通过这种方式绕过
    ...
    match=pattern.findall(str(soup))
    time.sleep(1)
    if (match.__len__() > 0):
        try:
...
def gethost(url):
    reg = urllib.request.Request(url, None, header)
    urlcontent = urllib.request.urlopen(reg).read()
    soup = BeautifulSoup(urlcontent, "html.parser")
...
def getMlist(inforL, daytime, daytime2, pagenum, urllist):
    param = urllib.parse.urlencode({'queryModel.currentPage': str(pagenum), 'queryModel.showCount': '10',
    ...
    for item in dicts["items"]:
        ...
        strtime = time.strftime('%m{0}%d{1} %H:%M',ttime).format(*'月日')
        reginner = urllib.request.Request(surl, None, header)
        soup = BeautifulSoup(urllib.request.urlopen(reginner).read(), "html.parser")
        ...
        if not addrtime(item.strtime,item.addr,item.company) in setlist:
            ...
            for info in filter(lambda items:items.time == item.strtime,setlist):
                ...
                if 1.0 > difflib.SequenceMatcher(lambda x: x in "医科集团股份有限公司", item.company, info.company).quick_ratio() > 0.7 and difflib.SequenceMatcher(None,item.addr, info.addr).quick_ratio() > 0.4:
                    print(item.strtime)
                    duplicate_str = input((item.company + "\t" + item.addr + " 是否与 " + info.company + "\t" + info.addr + " 重复？(输入y/n)"))
...
def upload(filename):#上传图片的函数，使用sm.ms图床
    filelist=[]
    filenamem = filename.replace("\\", "/")
    file=open(filenamem,'rb')
    filelist.append({'smfile': (filenamem, file, 'image/jpeg')})
    data = {'ssl': True}
    response = requests.post(uploadUrl,data=data, files=filelist[0])
    jsonre=response.json()
    if jsonre['code']=='success':
        return jsonre['data']['url'],jsonre['data']['delete']
    else:
        print(jsonre['msg'])
        return None,None
...
def qrcode_generate(url):
    version, level, qr_name = myqr.run(
        url,
        version=5,#size 1 to 40
        level='H',
        picture='logo.jpg',
        colorized=True,
        contrast=1.5,
        brightness=1.0,
        save_name=url.split('/')[-1].split("\\")[-1].split('=')[-1]+'.png',
        save_dir=os.getcwd()+'\\qrcode\\'
    )
    if os.path.getsize(qr_name)>0:
        return True,qr_name
    else:
        return False,None

def writeFileqr(List, handle,daytime):
    setlist = []
    outputstr = "<p style=\"line-height: 25.6px;white-space: normal;max-width: 100%;min-height: 1em;box-sizing: border-box !important;word-wrap: break-word !important;\"> <img src=\"http://mmbiz.qpic.cn/mmbiz_jpg/RW0pj7KFicIM2cn6MoBTaYC68esz4b8sRaDb4jz0Rv65dREib8rTq6ODcibPPgSVAv1wPxujykKLGaYjqmgqiceolg/640?wx_fmt=jpeg\" data-type=\"jpeg\" data-w=\"640\"f width=\"auto\" style=\"visibilityf: visible !important;width: auto !important;\"/><!--135不支持data-src的懒加载属性--></p>"  # 头图
    outputstr += "<section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"margin: 10px 0%;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;vertical-align: top;background-image: url(&quot;https://mmbiz.qpic.cn/mmbiz_jpg/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4TyJbKMMOPtib0zqaz0OhiaZOgqWFiaxywNZF4zmn0USwibibibNBFiboKg6heMw/640?wx_fmt=jpeg&quot;);background-position: 50% 50%;background-repeat: no-repeat;background-size: 100%;background-attachment: scroll;box-shadow: rgb(0, 0, 0) 0px 0px 0px;box-sizing: border-box;\"><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section></section></section><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"text-align: right;margin: 20px 0% 30px;overflow: hidden;font-size: 11px;transform: translate3d(0px, 0px, 0px);-webkit-transform: translate3d(0px, 0px, 0px);-moz-transform: translate3d(0px, 0px, 0px);-o-transform: translate3d(0px, 0px, 0px);box-sizing: border-box;\"><section style=\"width: 18em;height: 18em;margin-right: auto;margin-left: auto;text-align: center;border-radius: 100%;box-sizing: border-box;\"><section style=\"display: table;width: 100%;height: 100%;border-width: 2px;border-style: solid;border-color: transparent;border-radius: 100%;box-sizing: border-box;\"><section style=\"display: table-cell;vertical-align: middle;width: 60%;line-height: 1.6;padding: 15px;border-radius: 100%;box-shadow: rgb(116, 116, 116) 0px 0px 14px;background-color: rgb(255, 255, 255);box-sizing: border-box;\"><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"margin-top: 10px;margin-bottom: 10px;opacity: 0.5;box-sizing: border-box;\"><section style=\"max-width: 100%;vertical-align: middle;display: inline-block;width: 70%;box-sizing: border-box;\"><img data-ratio=\"0.1726384\" src=\"https://mmbiz.qpic.cn/mmbiz_png/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4TyfojZx8nFreNtkGkI8c7Her81JIfGQibIjWykkjp8MGQN2sS1osQp92w/640?wx_fmt=png\" data-type=\"png\" data-w=\"614\" style=\"vertical-align: middle; box-sizing: border-box; width: 100% !important; height: auto !important; visibility: visible !important;\" _width=\"100%\"  class=\"\" data-fail=\"0\"></section></section></section><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"box-sizing: border-box;\"><section style=\"color: rgb(245, 148, 135);font-size: 26px;letter-spacing: 0px;box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + time.strftime(
        "%m{0}%d{1}", time.strptime(daytime,
                                "%Y-%m-%d")).format(*'月日') + "</strong></p></section></section></section><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"box-sizing: border-box;\"><section style=\"color: rgb(245, 148, 135);font-size: 15px;box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">招聘会信息</strong></p></section></section></section><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"margin-top: 10px;margin-bottom: 10px;opacity: 0.51;box-sizing: border-box;\"><section style=\"max-width: 100%;vertical-align: middle;display: inline-block;width: 70%;box-shadow: rgb(0, 0, 0) 0px 0px 0px;box-sizing: border-box;\"><img data-ratio=\"0.1726384\" src=\"https://mmbiz.qpic.cn/mmbiz_png/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4TyfojZx8nFreNtkGkI8c7Her81JIfGQibIjWykkjp8MGQN2sS1osQp92w/640?wx_fmt=png\" data-type=\"png\" data-w=\"614\" style=\"vertical-align: middle; box-sizing: border-box; width: 100% !important; height: auto !important; visibility: visible !important;\" _width=\"100%\" class=\"\" data-fail=\"0\"></section></section></section></section></section></section></section></section><section class=\"\" style=\"box-sizing: border-box;\" ><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section></section></section></section></section></section>"  # 标题
    colors = [["rgb(145, 215, 206)", "rgba(129, 123, 123, 0.86)",
               "rgb(173, 225, 226);border-color: rgb(182, 235, 232);border-width: 6px;border-radius: 4px;border-style: none;box-shadow: rgb(0, 0, 0) 0px 0px 0px;width: 100%;line-height: 1.6;",
               "rgb(105, 219, 196)"],
              ["rgb(244, 210, 36)", "rgba(129, 123, 123, 0.86)", "rgb(240, 226, 125);border-width: 0px;",
               "rgb(255, 202, 0)", ],
              ["rgb(33, 43, 116)", "rgb(111, 106, 106)", "rgb(127, 130, 167);box-shadow: rgb(0, 0, 0) 0px 0px 0px;",
               "rgb(33, 43, 116)"]]
    handle.write(outputstr)
    theme = 0  # morning
    itemcount = 0
    count = List.__len__() > 5
    for item in List:
        if not addrtime(item.strtime, item.addr, item.company) in setlist:
            duplicate = False
            for info in filter(lambda items: items.time == item.strtime, setlist):
                ...
                if 1.0 > difflib.SequenceMatcher(lambda x: x in "医科集团股份有限公司", item.company, info.company).quick_ratio() > 0.7 and difflib.SequenceMatcher(None, item.addr, info.addr).quick_ratio() > 0.4:
                    print(item.strtime)
                    duplicate_str = input(
                        (item.company + "\t" + item.addr + " 是否与 " + info.company + "\t" + info.addr + " 重复？(输入y/n)"))
                    duplicate = duplicate_str.lower().__eq__(str('y'))
                    if duplicate:
                        break
            if duplicate:
                continue
            setlist.append(addrtime(item.strtime, item.addr, item.company))
            curr_time = item.ttime
            if (itemcount==0 and curr_time.tm_hour < 12):  # 只有当第一条在上午的时候才放太阳
                outputstr = "<section style=\"box-sizing: border-box;\"><section style=\"text-align: center;margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"max-width: 100%;vertical-align: middle;display: inline-block;width: 100%;box-sizing: border-box;\"><img data-ratio=\"0.4\" src=\"https://mmbiz.qpic.cn/mmbiz_jpg/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4TyZGydpOsdWZYNudaCfTXvrzUY4eicnKhQVJ6pkHFFbmJy3icTibPPWbYxQ/640?wx_fmt=jpeg\" data-type=\"jpeg\" data-w=\"500\" style=\"vertical-align: middle; box-sizing: border-box; width: 339px !important; height: 135.6px !important;\" _width=\"100%\"></section></section></section>"  # 太阳
                handle.write(outputstr)
            elif (curr_time.tm_hour > 12 and curr_time.tm_hour < 16 and theme != 1):  ##从上午切换到下午
                theme = 1  # afternoon
                ...
                outputstr = "<section style=\"box-sizing: border-box;\"><section style=\"text-align: center;margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"max-width: 100%;vertical-align: middle;display: inline-block;width: 100%;box-sizing: border-box;\"><img data-ratio=\"0.4\" src=\"https://mmbiz.qpic.cn/mmbiz_jpg/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4Ty9j0LYkpH8Rnke70tUZwf90qOZvz9LOE3RkbPLV5N012W6ySLUgbOSg/640?wx_fmt=jpeg\" data-type=\"jpeg\" data-w=\"500\" style=\"vertical-align: middle; box-sizing: border-box; width: 339px !important; height: 135.6px !important;\" _width=\"100%\"></section></section></section>"  # 月亮
                handle.write(outputstr)
            [qrres,qr_name] = qrcode_generate(item.surl)
            itemcount+=1
            if qrres:
                [url,delurl] = upload(qr_name)
                if url == None:
                    for i in range(3):
                        [url, delurl] = upload(qr_name)
                        if not url == None:
                            ...
                            time.sleep(1)
                if not url==None:
                    bufferstr = "<section style=\"box-sizing: border-box;\"><section style=\"margin: 10px 0%;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;vertical-align: top;border-color: " + colors[theme][0] + ";border-width: 2px;border-radius: 0px;border-style: dashed;box-shadow: rgb(0, 0, 0) 0px 0px 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"width: 100%;padding-bottom: 5px;background-color: " + colors[theme][1] + ";box-sizing: border-box;\"><section style=\"padding: 3px 10px;background-color: " + colors[theme][2] + "box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;border-width: 0px;border-style: solid;border-color: rgb(192, 200, 209);padding: 10px;background-color: " + colors[theme][0] + ";border-radius: 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: rgb(255, 255, 255);box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + (str(int(itemcount)) + "." if count else "") + item.company + "</strong></p></section></section></section></section></section></section></section></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.strtime + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.addr + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;font-size: 14px;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\">详情长按二维码识别</p><img style=\"width: 200px; height: 200px;\" src=\""+url+"\" alt=\""+url.split('/')[-1]+"\" title=\"213689442.jspx.png\"/></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section></section></section></section></section></section>"
                else:
                    print("上传图片失败")
                    bufferstr = "<section style=\"box-sizing: border-box;\"><section style=\"margin: 10px 0%;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;vertical-align: top;border-color: " + colors[theme][0] + ";border-width: 2px;border-radius: 0px;border-style: dashed;box-shadow: rgb(0, 0, 0) 0px 0px 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"width: 100%;padding-bottom: 5px;background-color: " + colors[theme][1] + ";box-sizing: border-box;\"><section style=\"padding: 3px 10px;background-color: " + colors[theme][2] + "box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;border-width: 0px;border-style: solid;border-color: rgb(192, 200, 209);padding: 10px;background-color: " + colors[theme][0] + ";border-radius: 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: rgb(255, 255, 255);box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + (str(int(itemcount)) + "." if count else "") + item.company + "</strong></p></section></section></section></section></section></section></section></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.strtime + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.addr + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;font-size: 14px;color: " + colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\">详情见："+item.surl+"</p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section></section></section></section></section></section>"
            else:
                print("二维码生成失败")
                bufferstr = "<section style=\"box-sizing: border-box;\"><section style=\"margin: 10px 0%;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;vertical-align: top;border-color: " +colors[theme][0] + ";border-width: 2px;border-radius: 0px;border-style: dashed;box-shadow: rgb(0, 0, 0) 0px 0px 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"width: 100%;padding-bottom: 5px;background-color: " +colors[theme][1] + ";box-sizing: border-box;\"><section style=\"padding: 3px 10px;background-color: " +colors[theme][2] + "box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"display: inline-block;width: 100%;border-width: 0px;border-style: solid;border-color: rgb(192, 200, 209);padding: 10px;background-color: " +colors[theme][0] + ";border-radius: 0px;box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: rgb(255, 255, 255);box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + (str(int(itemcount)) + "." if count else "") + item.company + "</strong></p></section></section></section></section></section></section></section></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " +colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.strtime + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;color: " +colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><strong style=\"box-sizing: border-box;\">" + item.addr + "</strong></p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"text-align: center;font-size: 14px;color: " +colors[theme][3] + ";box-sizing: border-box;\"><p style=\"box-sizing: border-box;\">详情见：" + item.surl+"</p></section></section></section><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section></section></section></section></section></section>"
            #bufferstr = item.company + "\n" + item.strtime + "\n" + item.addr + "\n岗位需求：\n详情见：" + item.surl + "\n\n"
            handle.write(bufferstr)
    outputstr = "<section style=\"box-sizing: border-box;\"><section style=\"margin-top: 10px;margin-bottom: 10px;text-align: center;box-sizing: border-box;\"><section style=\"display: inline-block;vertical-align: top;box-sizing: border-box;\"><section style=\"background-color: rgb(255, 255, 255);padding-right: 12px;padding-left: 12px;font-size: 14px;box-sizing: border-box;\"><p style=\"clear: none;box-sizing: border-box;\"><br style=\"box-sizing: border-box;\"></p></section><section style=\"width: 6px;height: 6px;border-radius: 100%;float: left;margin-top: -14px;background-color: rgb(15, 152, 226);box-sizing: border-box;\"></section><section style=\"width: 6px;height: 6px;border-radius: 100%;float: right;margin-top: -14px;background-color: rgb(15, 152, 226);box-sizing: border-box;\"></section><section style=\"clear: both;box-sizing: border-box;\"></section></section><section style=\"border-width: 2px;border-style: solid;border-color: rgb(15, 152, 226);margin-top: -12px;padding: 10px;width: 100%;box-sizing: border-box;\"><section class=\"\" style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"font-size: 14px;color: rgb(160, 160, 160);line-height: 3;box-sizing: border-box;\"><p style=\"box-sizing: border-box;\"><span style=\"letter-spacing: 0px;line-height: 3;box-sizing: border-box;\"> 编辑 / 自行修改</span></p></section></section></section></section></section></section><section class=\"\" style=\"box-sizing: border-box;\"><section style=\"text-align: center;margin-top: 10px;margin-bottom: 10px;box-sizing: border-box;\"><section style=\"max-width: 100%;vertical-align: middle;display: inline-block;box-sizing: border-box;\"><img data-ratio=\"0.4734375\" data-type=\"jpeg\" data-w=\"640\" style=\"vertical-align: middle; box-sizing: border-box; width: 640px !important; height: auto !important; visibility: visible !important;\" _width=\"640px\" class=\"\" src=\"https://mmbiz.qpic.cn/mmbiz_jpg/RW0pj7KFicIOMxrQQcLICJnNLV1gMk4TyFsPszXib2xHAhBd2zhZiaMUEPKIQkkMQfO0aB7icpibNUE8d0MZCMGf9Tg/640?wx_fmt=jpeg&amp;tp=webp&amp;wxfrom=5&amp;wx_lazy=1&amp;wx_co=1\" data-fail=\"0\"></section></section></section><section class=\"\" style=\"box-sizing: border-box;\"><section style=\"box-sizing: border-box;\"><section style=\"border-bottom: 2px solid rgb(95, 156, 239);font-size: 14px;box-sizing: border-box;\"><p style=\"box-sizing: border-box;\">点击 “阅读原文” 了解详情</p></section></section></section><section class=\"\" style=\"box-sizing: border-box;\"><section style=\"margin-left: 8%;margin-right: 8%;line-height: 1.1em;box-sizing: border-box;\"><section style=\"width: 0px;display: inline-block;vertical-align: top;border-top: 1em solid rgb(95, 156, 239);border-left: 0.8em solid transparent !important;border-right: 0.8em solid transparent !important;box-sizing: border-box;\"></section></section></section>"
    handle.write(outputstr)

# 用户交互&顶层函数
#[qrresult,qrname] = qrcode_generate('http://job.hust.edu.cn/extxjh/213689442.jspx')
infostr = ""
inforL = []
urllist = []
days = input("输入w/W获取下周招聘格式化信息，输入数字获得指定天数之后的招聘文字信息，输入日期如：2018-09-17格式化指定文本文档的信息，直接回车默认输入2")
...
if ((days == 'w') | (days == 'W')):
    ...
    writeFileqr(sortL, handle,daytimestr2)
handle.close()
print("运行完成")